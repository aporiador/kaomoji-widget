...
use crate::monitor::{get_monitors, is_position_on_any_monitor};
use crate::settings::{get_settings, set_settings};
use crate::tray::setup_tray;
use monitor::{collect_monitors, get_notch_inset};
use serde_json::json;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let monitor_cache = collect_monitors(app.handle().clone())?;
            ...
            if let Some(window) = app.get_webview_window("main") {
                ...
                if settings.notch_mode {
                    ...
                    // Restore saved floating position if it lands on a connected monitor
                ...
                tauri::async_runtime::spawn(async move {
                    loop {
                        ...
                        // Don't overwrite the floating position while in notch mode
                        let in_notch = store_for_save
                            .get("widget_settings")
                            .and_then(|v| {
                                serde_json::from_value::<crate::settings::Settings>(v).ok()
                            })
                            .map(|s| s.notch_mode)
                            .unwrap_or(false);
                        if in_notch {
                        ...
                        if let Some(window) = app_handle_for_save.get_webview_window("main") {
                            ...
                            let logical_pos: tauri::LogicalPosition<f64> =
